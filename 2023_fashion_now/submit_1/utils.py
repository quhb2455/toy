...
def MultiHead_cutmix(imgs, labels):
    lam = np.random.beta(1.0, 1.0)
    rand_index = torch.randperm(imgs.size()[0]).cuda()
    target_a = labels
    target_b0 = labels[0][rand_index]
    target_b1 = labels[1][rand_index]
    target_b2 = labels[2][rand_index]
    bbx1, bby1, bbx2, bby2 = rand_bbox(imgs.size(), lam)
    imgs[:, :, bbx1:bbx2, bby1:bby2] = imgs[rand_index, :, bbx1:bbx2, bby1:bby2]

    lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (imgs.size()[-1] * imgs.size()[-2]))

    return imgs, lam, target_a, [target_b0, target_b1, target_b2]
...
def MultiHead_mixup(imgs, labels):
    lam = np.random.beta(1.0, 1.0)
    rand_index = torch.randperm(imgs.size()[0]).cuda()
    mixed_imgs = lam * imgs + (1 - lam) * imgs[rand_index, :]
    target_a = labels
    ...
    target_b1 = labels[1][rand_index]
    target_b2 = labels[2][rand_index]
    return mixed_imgs, lam, target_a, [target_b0, target_b1, target_b2]
