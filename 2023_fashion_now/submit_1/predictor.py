...
class Predictor() :
    ...
    def prediction(self, **cfg) :
        ...
        with torch.no_grad() :
            for img in tqdm(self.test_loader) :
                d_pred, g_pred, e_pred = self.predict_on_batch(img, **cfg)
                
                d_model_preds.extend(d_pred)
                g_model_preds.extend(g_pred)
                e_model_preds.extend(e_pred)
                
        return self.save_to_csv([d_model_preds, g_model_preds, e_model_preds], **cfg)
    ...
    def predict_on_batch(self, img, **cfg) :
        ...
        return d_output, g_output, e_output
