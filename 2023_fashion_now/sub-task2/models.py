...
class RGBModel(nn.Module) :     
    ...
    def forward(self, x):
        ...
        output = self.model.forward_head(emb)
        return emb, output
...
class RGBCalssifierHead(nn.Module) :
    def __init__(self, base_dim, mid_dim, num_classes) :
        super(RGBCalssifierHead, self).__init__()
        self.head = FullyConnectedLayer(
            base_dim=base_dim, 
            mid_dim=mid_dim, 
            num_classes=num_classes)
        self.ca = ChannelAttention(in_ch=base_dim) 
    
    def forward(self, r, g, b) :
        rgb = torch.cat([r, g, b], dim=1)
        rgb = self.ca(rgb)
        output = self.head(rgb)
        return output
